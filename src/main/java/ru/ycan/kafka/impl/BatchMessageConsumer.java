...
@Slf4j
public class BatchMessageConsumer implements MessageConsumer {
        ...
        try (var consumer = new KafkaConsumer<String, byte[]>(props)) {
            ...
            while (true) {
                try {
                    ...
                    // Подтверждаем смещение коммита
                    consumer.commitSync();
                    log.info("Обработано '{}' сообщений", records.count());
                } catch (Exception e) {
                    log.error("Не удалось вычитать/обработать сообщения", e);
                }
            }
        }
    }
...
//        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9092,localhost:9093,localhost:9094");
        ...
        // Определяет минимальный объём данных (в байтах)
        // руками пришёл к выводу, что где-то около 2625 в 10 сообщениях, но это не точно
        props.put(ConsumerConfig.FETCH_MIN_BYTES_CONFIG, "2625");
        // Определяет максимальное время ожидания
        props.put(ConsumerConfig.FETCH_MAX_WAIT_MS_CONFIG, "10000");
        return props;
    }

    /**
     * Проверка кол-ва сообщений (скорее всего последние логи будут отсюда, т.к. не факт что всё обработается пачками кратными 10).
     *
     * @param records сообщения из kafka
     * @return true, если 10 сообщений и более
     */
    private boolean checkRecordsCount(ConsumerRecords<?, ?> records) {
        if (records.isEmpty()) {
            return false;
        }
        if(records.count() > 0 && records.count() < 10) {
            log.warn("Кол-во сообщений '{}' меньше 10, поправьте конфигурацию '{}'",
                     records.count(), ConsumerConfig.FETCH_MIN_BYTES_CONFIG);
            return false;
        }
        return true;
    }
